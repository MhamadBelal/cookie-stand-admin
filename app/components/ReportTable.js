...
function ReportTable({ dataResponse, hours, DleteData }) {
                            ...
                            {dataResponse.map((dataResponse, index) => (
                                <tr key={index}>
                                    <td className="flex border p-3 border-black justify-between" ><span>{dataResponse.location}</span>
                                        <button type="submit" onClick={DleteData(dataResponse.Id)}> <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5 bg-red-500">
